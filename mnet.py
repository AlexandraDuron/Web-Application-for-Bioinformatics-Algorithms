...
def main(argv):
    ...
    # pass the options database throught the fast_cosine and fast_cosine_shift funtions
    ...
    # first stage of mass analysis (MS1)
    # detected in the second stage (MS2)   MA1 and MS2 are first and second analysis of mass.
    ms1,spectra = load_spectra(input_dir,options)

    # ms1 = nombre del script , spectra = min_ms2_intensity
    ...
    #read a csv file and and put the data in a "row" array , then print an output file
    print()
    print()
...
#load espectra with the data base and the array
...
# Class to hold a cluster of spectra, define a single gruop of spectra
...
# busca se el spectrum es simlar a algune grupo en la lista
def merge(cluster_list,spectrum,similarity_function,similarity_tolerance,min_match,score_threshold = 0.95,rt_tolerance = 1000000,ms1_tolerance=0.02,initial_cluster_id = 0):
    # Compute the similarity between the spectrum and all clusters in the list
